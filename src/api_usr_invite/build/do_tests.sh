...
metaMongoDbServiceName="rf-svc-usermgr-mongo-db-test"
metaMongoDbServiceDb="aribor_user_db"
...
run_tests () {
  ...
  cd ${SCRIPT_PATH}
  GOPATH=$HOME/go GOBIN=$HOME/go/bin DB_MONGO_USR=$(get_password rf-svc-usermgr-mongo-db-usr) DB_MONGO_PWD=$(get_password rf-svc-usermgr-mongo-db-pwd) DB_MONGO_LNK=mongodb://localhost:${metaMongoDbLocalTargetPort} DISABLE_DEBUG=1 go test ../ -mod=readonly -v
  cd -
